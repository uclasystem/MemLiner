...
inline struct rswap_rdma_queue *get_rdma_queue(unsigned int cpu, enum rdma_queue_type type)
{
	...
	case QP_ASYNC:
		return &global_rdma_ctx->rdma_queues[cpu + ONLINE_CORES];
	default:
		printf("wrong rdma queue type %d\n", type);
	};
	return &global_rdma_ctx->rdma_queues[cpu];
...
int main(int argc, char *argv[])
{
	...
	inet_pton(AF_INET6, ip_str, &addr.sin6_addr); // Remote memory pool is waiting on ip_str:port_str
...
void init_memory_pool(struct context *rdma_ctx)
{
	...
	rdma_ctx->mem_pool->region_mapped_size[0] = (size_t)(REGION_SIZE_GB * ONE_GB);
